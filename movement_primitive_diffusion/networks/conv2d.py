...
from movement_primitive_diffusion.networks.mlp import MLP
...
class Conv2dModel(torch.nn.Module):
    ...
    def __init__(
        ...
        if isinstance(nonlinearity, str):
            nonlinearity: torch.nn.Module = getattr(torch.nn, nonlinearity)
        if paddings is None:
...
class Conv2dHeadModel(torch.nn.Module):
    ...
    def __init__(
        ...
        if hidden_sizes or output_size:
            self.head = MLP(conv_out_size, hidden_sizes, output_size=output_size, hidden_nonlinearity=nonlinearity)
